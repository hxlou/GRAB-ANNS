...
int main(int argc, char** argv) {
    if (argc < 2 || argc > 3) {
        std::cerr << "Usage: " << argv[0]
                  << " DEEP_BASE.fvecs [output.csv]" << std::endl;
        return 2;
    }
    ...
    const std::string csv_file =
        (argc == 3) ? argv[2] : "results/exp2_ablation/grab.csv";

    std::vector<float> host_data;
    int dim = 0;
    size_t file_total = 0;
    load_fvecs(data_path, host_data, dim, file_total);
    if (dim != 96) {
        std::cerr << "Warning: DEEP-96 expected, got dimension " << dim
                  << std::endl;
    }

    const size_t n = std::min<size_t>(1000000, file_total);
    const std::vector<double> ranges = {0.01, 0.10, 0.20, 1.00};

    auto evaluate = [&](const BuildConfig& build,
                        const std::vector<SearchConfig>& searches) {
        const size_t bucket_size = build.total_data_size / build.num_buckets;
        std::vector<uint64_t> timestamps(build.total_data_size);
        for (size_t i = 0; i < build.total_data_size; ++i) {
            timestamps[i] = i / bucket_size;
        }

        cagra::CagraIndexOpt index(dim, build.graph_degree);
        index.setBuildParams(build.graph_degree * 2, build.graph_degree);
        Timer timer;
        index.add(build.total_data_size, host_data.data(), timestamps.data());
        index.build();
        const double build_time = timer.elapsed_ms();
        auto tasks = generate_tasks(host_data.data(), dim, build, ranges);
        for (const auto& search : searches) {
            run_range_benchmark_fast(
                index, dim, build, search, build_time, tasks, csv_file);
        }
    };

    // Graph-degree sweep: K=128 and max_iterations=100.
    for (uint32_t degree : {32u, 64u, 96u, 128u}) {
        evaluate({n, 100, degree}, {{128, 4, 100}});
    }

    // itop_k and max-iteration sweeps with M=64.
    evaluate(
        {n, 100, 64},
        {
            {64, 4, 100},
